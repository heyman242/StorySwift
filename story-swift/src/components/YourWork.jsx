import React from "react";
import { IoIosCamera } from "react-icons/io";
import { HiOutlineAdjustments } from "react-icons/hi";
import { FaTiktok, FaSquareInstagram, FaSquareYoutube } from "react-icons/fa6";

const YourWork = () => {
  return (
    <section class=" bg-[#090517]">
      <div class="container px-6 py-10 mx-auto">
        <div class="lg:flex lg:items-center">
          <div class="w-full space-y-12 lg:w-1/2 ">
            <div>
              <h1 class="text-2xl font-semibold text-gray-800 capitalize lg:text-3xl dark:text-white">
                Easy as 1-2-3: Simplified &nbsp;
                <span className="text-[#fce54d] underline">3 Step Process</span>
                &nbsp;.
              </h1>

              <div class="mt-2">
                <span class="inline-block w-40 h-1 bg-[#fce54d] rounded-full"></span>
                <span class="inline-block w-3 h-1 ml-1 bg-[#fce54d] rounded-full"></span>
                <span class="inline-block w-1 h-1 ml-1 bg-[#fce54d] rounded-full"></span>
              </div>
            </div>

            <div class="md:flex md:items-start md:-mx-4">
              <span class="inline-block p-1 text-blue-500 bg-blue-100 rounded-xl md:mx-4 dark:text-white dark:bg-[#090517]">
                <IoIosCamera size={30} />
              </span>

              <div class="mt-4 md:mx-4 md:mt-0">
                <h1 class="text-xl font-semibold text-gray-700 capitalize dark:text-[#fce54d]">
                  Get on Camera
                </h1>

                <p class="mt-3 text-gray-500 dark:text-gray-300">
                  Allocate time for recording, and let us handle the rest. we
                  will develop video ideas and scripts in advance. We'll guide
                  you through best practices for recording videos that drive
                  performance and conversions.
                </p>
              </div>
            </div>

            <div class="md:flex md:items-start md:-mx-4">
              <span class="inline-block p-1 text-blue-500 bg-blue-100 rounded-xl md:mx-4 dark:text-white dark:bg-[#090517]">
                <HiOutlineAdjustments size={30} />
              </span>

              <div class="mt-4 md:mx-4 md:mt-0">
                <h1 class="text-xl font-semibold text-gray-700 capitalize dark:text-[#fce54d]">
                  We Edit The Videos
                </h1>

                <p class="mt-3 text-gray-500 dark:text-gray-300">
                  Watch your recordings come to life as we transforms them into
                  both long and short-form videos. Enjoy a steady stream of
                  content, allowing you to post daily or multiple times a day.
                </p>
              </div>
            </div>

            <div class="md:flex md:items-start md:-mx-4">
              <span class="inline-block p-1 text-blue-500 bg-blue-100 rounded-xl md:mx-4 dark:text-white dark:bg-[#090517]">
                <FaTiktok size={30} />
                <FaSquareInstagram size={30} />
                <FaSquareYoutube size={30} />
              </span>

              <div class="mt-4 md:mx-4 md:mt-0">
                <h1 class="text-xl font-semibold text-gray-700 capitalize dark:text-[#fce54d]">
                  Propagate Onto All Platforms
                </h1>

                <p class="mt-3 text-gray-500 dark:text-gray-300">
                  Leave the distribution to us. We'll take your short-form
                  videos and share them across all platforms. Sit back, relax,
                  and witness the views and conversions roll in effortlessly.
                </p>
              </div>
            </div>
          </div>

          <div class="hidden lg:flex lg:items-center lg:w-1/2 lg:justify-center">
            <img
              class="w-[28rem] h-[28rem] object-cover xl:w-[34rem] xl:h-[34rem] rounded-full"
              src="https://images.unsplash.com/photo-1497015289639-54688650d173?q=80&w=1632&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt=""
            />
          </div>
        </div>

        <hr class="my-12 border-gray-200 dark:border-gray-700" />
      </div>
    </section>
  );
};

export default YourWork;
