...
const SocialLinks = () => {
  const links = [
...

    {
      id: 2,
      name: "   Mail",
      icon: <FaSquareEnvelope size={50} color="#1da1f2" />,
      ...
      <ul className="hidden lg:block">
        {links.map(({ id, name, icon, href, style, download }) => (
          <li
            key={id}
            className={
              "flex justify-between items-center w-40 h-16 px-3 ml-[-90px] hover:ml-[-10px] hover:rounded-md duration-300 bg-[#ffffff]" +
              " " +
              style
            }
          >
